...
bool TestRayInFrustrum( const CVec3 &ptPos, CTransformStack *pTS, const CVec2 &screenRect, CVec2 *pRes = 0 );
...
{
	CVec4 viewFrustrum[6];
	BYTE nClipFlags[32];
	int nClipFlagsPtr;
	//
	void PrepareClipPlanes();
	bool CheckSphere( const CVec4 &ptCenter, float fRadius, bool *pRes );
	bool CheckParallelepiped( const CVec4 &ptCenter, const CVec3 &ptXAxis, const CVec3 &ptYAxis, const CVec3 &ptZAxis );
public:
