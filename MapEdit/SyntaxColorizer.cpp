...
//
// Version:	1.0.0
// Author:	Jeff Schering [email removed]
// Date:	Jan 2001
...
{
	CObj<SKeyword> pskNewKey = new SKeyword;
	CObj<SKeyword> prev, curr;
	...
	*(m_pTableZero + pskNewKey->keyword[0]) = KEYWORD;
	...
	else
	{
		...
		if(strcmp(Keyword,m_pskKeyword->keyword.c_str()) < 0)
		{
			pskNewKey->pNext = m_pskKeyword;
			m_pskKeyword = pskNewKey;
		}
		//check to see if new keyword already exists at the first node
		else if(strcmp(Keyword,m_pskKeyword->keyword.c_str()) == 0)
		{
		...
		else
		{
			...
			while(curr != NULL && strcmp(curr->keyword.c_str(),Keyword) < 0)
			{
				prev = curr;
				curr = curr->pNext;
			}
			if(curr != NULL && strcmp(curr->keyword.c_str(),Keyword) == 0)
			{
				//the keyword exists, so replace the existing with the new
...
void CSyntaxColorizer::ClearKeywordList()
{
	while(m_pskKeyword != NULL)
	{
		*(m_pTableZero + m_pskKeyword->keyword[0]) = SKIP;
		if(_stricmp(m_pskKeyword->keyword.c_str(),"rem") == 0)
			*(m_pTableTwo + '\n') = SKIP;
		m_pskKeyword = m_pskKeyword->pNext;
	}
}
...
{
	CString sList;
	CObj<SKeyword> pTemp = m_pskKeyword;

	while(pTemp != NULL)
	{
		sList += pTemp->keyword.c_str();
		sList += ",";
		pTemp = pTemp->pNext;
	}
	sList.TrimRight(',');
...
void CSyntaxColorizer::colorize(LPTSTR lpszBuf, CRichEditCtrl *pCtrl, long iOffset /*=0*/)
{
	...
	long iStart;
	long x = 0;
	CObj<SKeyword> pskTemp = m_pskKeyword;
		...
		case KEYWORD:
			...
			while(pskTemp != NULL)
			{
				if(pskTemp->keyword[0] == lpszTemp[0])
				{
					...
					{
						if(_stricmp(pskTemp->keyword.c_str(),"rem") == 0)
						{
							pTable = m_pTableThree;
						}
						else 
